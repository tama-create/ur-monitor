...
コードを触るときに必要な前提だけを書く。資料の一覧と役割は資料の入口（`docs/index.html`）にあり、
...
`output/index.html`。どちらも .gitignore 済み）に落ちる。**`docs/index.html` は資料の入口なので、
一覧の書き出し先をそこへ戻さないこと**（開発実行のたびに入口が上書きされる）。**開発中に秘密情報なしで動かせるようにするための逃げ道**で、
