...
主要な流れは `run_monitor()`（`ur_monitor.php:1211`）に集約されている。
...
ただし心拍だけでは足りなかった。2026-09-13 13:10 の実行がランナーをもらえず「待ち」のまま
固まり、concurrency の枠を24時間握り続けた。後続の実行は `schedule` も含めてすべて押し出されて
キャンセルされ、`warn_if_stale` も実行されないので、**24時間後に枠が空くまで誰も気づけなかった**
（run 34737267050。ステップ0件・ランナー無し。`timeout-minutes` は開始後しか効かない）。
そこで `trigger/worker.js` が起動のたびに、20分以上終わらない実行をキャンセルし、
最後の成功から稼働時間帯で30分空いたら **GitHub の外から** Slack へ知らせるようにした。
**見張りを Actions の中だけに置かないこと。** 止まるときは見張りごと止まる。
...
- ただし持ち主はコードを読まず、次の3つで全体を把握している。**処理を変えたら、これらも必ず合わせて直すこと**
  - `FLOW.md`：モードごとの詳しい処理の流れと、Mermaid のフローチャート
  - `ur_monitor.php` 冒頭の PHPDoc：モードとあらすじだけを**簡潔に**（詳細は FLOW.md に置き、ここを長くしない）
  - すべての関数に JavaDoc 形式（PHPDoc）で、何をするかを1〜2行と `@param`・`@return`（手順の羅列は書かない）
- `.editorconfig` に従う
