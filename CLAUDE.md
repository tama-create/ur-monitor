...
`ur_monitor.php` 1ファイル（コメント込みで約2,000行）に全処理があり、フレームワークは使っていない。
...
主要な流れは `run_monitor()`（`ur_monitor.php:1664`）に集約されている。
...
- ただし `ur_monitor.php` には、その例外として次の2つを置いている。持ち主はコードを読まず、
  これを見て全体を把握するため。**処理を変えたら、この2つも必ず合わせて直すこと**
  - ファイル冒頭の PHPDoc に、番号付きの詳しい「処理の流れ」（モードごと）
  - すべての関数に JavaDoc 形式（PHPDoc）で、何をするか・手順・`@param`・`@return`
- `.editorconfig` に従う
