...
5分おきに GitHub Actions の監視ワークフローを起動する。
...
UTC で5分おきに起動し、稼働時間帯（JST 08:00〜21:00）の判定は `worker.js` の中で行う。
無料プランは**1ワーカーあたり cron 3本まで**なので、1本で済ませて余裕を残してある。
呼び出し回数は1日288回で、無料枠（10万回／日）に対して誤差の範囲。
