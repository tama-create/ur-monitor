...
起動を依頼する前に、次の2つを行う。どちらも失敗したらログに残すだけで、**起動は必ず出す**。

| 見回り | 内容 |
|---|---|
| 詰まった実行の片付け | 20分たっても終わっていない実行をキャンセルする（40分を過ぎたら強制キャンセル） |
| 止まりの見張り | 最後に成功した実行から稼働時間帯で30分以上空いたら Slack に知らせる（同じ停止について1回だけ） |

**なぜ要るのか**：`monitor.yml` は同時に1本しか走らせない（concurrency）。ある実行が
ランナーをもらえず「待ち」のまま固まると枠を握り続け、後から来た実行は全部キャンセルされる。
保険の `schedule` も同じ枠なので一緒に止まり、`warn_if_stale` は実行が成功しないと鳴らせない。
`timeout-minutes` はジョブが始まってからしか効かず、待ちは GitHub が24時間後に打ち切るまで残る。
2026-09-13 13:10 の実行がこうなり、**約24時間・290回ぶん止まったうえ、24時間後まで誰も気づけなかった**。
...
連投を防ぐ印は KV（`STORE`）に `stale_alerted` という名前で置く。
