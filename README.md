...
- 志望順位の高いグループに合致する新着があれば Slack へ通知
- 実行結果（前回の状態と一覧 HTML）は Cloudflare Worker の KV へ置く。**リポジトリには
  戻さない**（Actions のランナーは毎回まっさらでファイルが残らないため、どこかに預ける
  必要はある。GitHub ではなく Cloudflare にしている理由は下記）
...
`--dry-run` を付けずに実行すると本番と同じ動作になる。`STORE_URL` / `STORE_TOKEN` を
設定していれば**本番の保管先を直接書き換える**ので、手元では設定しないこと。
...
| キー | 必須 | 意味 |
|---|---|---|
| `groups` | ✓ | 志望順位ごとの監視対象。**ここに並べた URL に出てくる物件だけが監視対象**になり、グループごとに通知するかを決める |
...
| `docs_base_url` |  | 資料ページの公開先。一覧の上部メニューからここへリンクする。**空なら資料へのリンクを出さない**（一覧は Worker から配るので、相対パスだと行き先が無い） |
...
**大学受験の第一志望・滑り止めと同じ考え方**で、入居したい物件ほど上に置く。
グループごとに「通知するか」「どの間取りなら通知するか」を決められるので、
*本当に入りたい物件だけ Slack を鳴らし、相場を知りたい地域は一覧に出すだけ*にできる。
...
**同じ部屋が複数のグループに出てきたときは、上のグループが勝つ。** 団地ページを第一志望に、
その団地を含む地域ページを滑り止めに置いても、その団地の部屋は第一志望として通知される。
だから「本命の団地ページ＋周辺の地域ページ」という並べ方をそのまま書いてよい。

間取りの照合は**部分一致**。`"2DK"` は `2DK / 51㎡ / 4階` に一致するが、`2LDK` には一致しない。

#### 旧形式（search_urls + watch）

以前の形式もそのまま動く。`groups` が無いときだけ読まれる。
...
この書き間違いの問題自体が無くなる。新しく設定するなら `groups` を使うほうがよい。
...
**一覧も前回状態も GitHub にはコミットしない。** Cloudflare Worker の KV に置き、
閲覧は Basic 認証の内側に限っている。

UR は「[このサイトについて](https://www.ur-net.go.jp/site/guide.html)」で、私的使用と
引用を除く転載を認めていない。そのうえで**「インターネットのホームページに写真等を
コピーして貼り付けることは、その運営者が個人であっても私的使用にはなりません」**と
明示している。物件名・家賃・間取りを公開ページや公開リポジトリに並べるのは、この線をまたぐ。

事実の羅列に著作物性が及ぶかには議論があり、違法と確定した話ではない。ただ
**UR 自身がそう書いている以上、白いほうを選ぶ**という判断。以前は `docs/index.html` を
GitHub Pages で公開していたが、やめた。
...
定まっておらず、日本語や記号だと正しく入力しても弾かれる環境があるため。

### 保管先を用意した直後は1回だけ移送が要る

保管先が空だと「前回は0件」になり、**いま出ている部屋が全部 Slack に飛ぶ**。
`load_state()` はそれを避けて**中止する**ので、まず移送する。
...
いずれも作者が書いた文章で、UR の内容は含まない。有効化の手順は下記
「セットアップ手順書の公開（GitHub Pages）」と共通。

生成する一覧 HTML にも `noindex, nofollow` を入れてある。認証の内側なので本来は
不要だが、取り違えて公開したときの保険。
...
実行してくれるわけではない**。`schedule` は負荷状況に応じて遅延し、ときには発火自体が
破棄される。いま間隔を守っているのは Cloudflare Workers 側で、`monitor.yml` に残した
`schedule` は1日4回の心拍だけ。以下はそう決めるに至った実測。
...
**9時間で18回動くはずが12回**しか動いていなかった。約3分の1が欠落し、実効間隔は50分前後だった。
この数字は当時のコミット履歴から数えたもので、実行のたびに一覧をコミットしていたため
履歴がそのまま全実行記録になっていた。**いまはコミットしないので履歴からは追えない。**
実行頻度を疑うときは Actions タブの実行一覧を見ること。
...
監視が止まっていないかを見張る心拍を兼ねる。**この4回を「間隔」として当てにしないこと。**

Cloudflare に移したあとの実測（2026-08-28）。15分設定のときは5回連続で15分ちょうど・
毎回同じ秒、5分設定に変えたあとも5分ちょうどで並んだ。GitHub の `schedule` で
46〜64分にばらついていたのとは別物になっている。
...
- **公開リポジトリなら Actions は無料枠の対象外（無制限に無料）**。以下はプライベートに
  戻す場合の話。無料枠は月2,000分で、5分間隔は1日168回・月約5,000回。
  1回あたりの実測は45〜85秒（＋jitter 最大40秒）で、課金はジョブごとに分単位へ
  切り上げられるため **月およそ7,500分**になり、無料枠を大きく超える。
...
- `jitter_max_seconds` は 40。以前は 0 だった。GitHub の `schedule` は実行時刻が
  勝手にゆらいでいたのでゆらぎを足す必要が無く、待機時間がそのまま課金対象に
  なるためでもあった。Cloudflare から起動するようになって**毎回きっかり同じ秒に
  発火するようになった**ため、ゆらぎは自分で入れる必要が出た。公開リポジトリの
