...
```mermaid
flowchart TD
    A([開始]) --> B["1. 設定を読む<br>load_config"]
    B --> C["2. 監視対象を整える<br>normalize_groups / group_url_map"]
    C --> C2{監視対象がある?}
    C2 -- いいえ --> X1([終了コード1])
    C2 -- はい --> D["3. 0〜40秒ランダムに待つ"]
    D --> E["4. 前回の状態を読む<br>load_state"]
    E --> E2{読めた?}
    E2 -- いいえ --> X2([終了コード1])
    E2 -- はい --> F["5. 止まっていなかったか確認<br>warn_if_stale"]
    F --> G["6. Chrome を起動<br>create_browser"]
    G --> H["7. URL ごとに取得<br>（下の図）"]
    H --> I["8. Chrome を閉じる"]
    I --> J{"信用できる結果が<br>1つでもあった?"}
    J -- いいえ --> Y([何も書かずに終了])
    J -- はい --> K["10. 前回と比べて新着を出す"]
    K --> L["11. 通知すべき新着を Slack へ<br>room_notifies / notify_watch"]
    L --> M{--dry-run?}
    M -- はい --> Z([終了])
    M -- いいえ --> N["13. 一覧ページを書く<br>save_html"]
    N --> O["14. 今回の状態を書く<br>save_state"]
    O --> Z2([終了])
```
...
1. すべての監視 URL について robots.txt を確認する
...
終了コードは、0 が正常（前回状態を維持してスキップした場合も含む）、1 が設定・前回状態・書き込みの異常、または想定外の例外です。

**判断の基準**：通知を1回逃すより、誤った通知を飛ばすほうが害が大きい。そのため「怪しい結果は採用しない」「前回状態が分からなければ止まる」を優先しています。
